...
- **Agent file edits — use Edit not Write**: Background agents must use the `Edit` tool (targeted old→new patches) to modify existing files. The `Write` tool (whole-file overwrite) requires interactive user approval which background agents cannot receive, causing all writes to be denied silently. Since worktree files are copies of existing repo files, `Edit` always applies cleanly.
